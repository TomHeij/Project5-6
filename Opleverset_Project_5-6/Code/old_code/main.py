...
class MainApp(QtWidgets.QMainWindow):
    ...
    def _update_map_from_detections(self):
        ...
        try:
            ...
            # 4. Update map view with filtered objects
            if self.main_content and hasattr(self.main_content, 'map_view'):
                # Prepare objects list: (name, distance, pixel_x, obj_id)
                objects_to_display = []
                for pair in bound_pairs:
                    (xL, yL), (xR, yR), cls_id, class_name, confidence, distance = pair
                    ...
                    # Get object ID from tracking
                    obj_id = self._get_or_create_object_id((xL, yL), match_threshold)
                    
                    # Apply filters
                    if class_name == "drone" and not self.filter_state["drones"]:
                        continue
                    if class_name == "ship" and not self.filter_state["ships"]:
                        continue
                    if class_name not in ("drone", "ship") and not self.filter_state["unknown"]:
                        continue
                    
                    objects_to_display.append((class_name, distance, xL, obj_id))
                
                # Update map with unified function
                self.main_content.map_view.update_map(
                    objects_to_display,
                    pixel_to_meter_func=self._pixel_to_meter
                )
